...
const TRUSTED_SPEND_SOURCES = new Set([
  'forge_frame',
  'forge_aura',
  'forge_ship_skin',
  'forge_title',
...
function registerSpendRoute(ctx) {
  ...
  return async function handleSpendRoute(req, res, url, pathname) {
    ...
    try {
      ...
      const sanitizedSource = typeof source === 'string' ? source.slice(0, 50) : '';
      if (!TRUSTED_SPEND_SOURCES.has(sanitizedSource)) {
        return respondJson(res, 400, { error: 'Invalid spend source' });
      }
      ...
      try {
        ...
        if (sanitizedSource.startsWith('forge_')) invalidateQuestProgressCache(address);
      ...
      const tx = {
        ...
        description: `Spent ${spent} Coins on ${sanitizedSource.replace(/^forge_/, 'forge ').replace(/_/g, ' ')} (${burned} burned)`,
      ...
      if (sanitizedSource === 'forge_module') {
        const moduleDef = forgeModuleMap.get(moduleId);
        if (moduleDef) {
          pushNotification(address, 'system', `Purchased module: ${moduleDef.name} (-${spent} Coins)`, {
            source: sanitizedSource,
            moduleId,
            spent,
          });
        ...
        const itemDef = forgeItemMap.get(itemId);
        if (itemDef) {
          pushNotification(address, 'system', `Purchased ${itemDef.name} (-${spent} Coins)`, {
            source: sanitizedSource,
            itemId,
            spent,
          });
