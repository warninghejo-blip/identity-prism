...
function registerEarnRoute(ctx) {
  ...
  const { questSourceIds, getQuestProgressSnapshot, invalidateQuestProgressCache = () => {}, quests, saveQuestProgressDebounced, validTextQuestIds, questXpRewards = {} } =
    quest;
      ...
      if (GAME_EARN_SOURCES.has(source)) {
        ...
        if (source === 'quest_daily' || source === 'quest_weekly' || source === 'quest_milestone') {
          ...
          try {
            ...
            const totalXPEarned = Math.max(0, Number(existingQuestState.totalXPEarned) || 0) + (questXpRewards[questId] || 0);
