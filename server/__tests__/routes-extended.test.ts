...
const ADDRESSES = {
  ...
  suspicious: '33333333333333333333333333333333',
  target: '33333333333333333333333333333334',
};
...
const seedWorkspace = () => {
  ...
  writeJson(path.join(metadataDir, 'sybil-graph.json'), {
    nodes: {
      [ADDRESSES.suspicious]: {
        riskScore: 72,
        trustGrade: 'D',
        siblings: [ADDRESSES.target],
        verdictKey: 'probable_sybil',
        lastSeen: Date.now(),
      },
      [ADDRESSES.target]: {
        riskScore: 50,
        inferredFromCluster: ADDRESSES.suspicious,
        lastSeen: Date.now(),
      },
    },
    flaggedClusters: [],
...
describe.sequential('routes-extended integration tests', () => {
  ...
  it('GET /api/daily-limits without auth returns 401', async () => {
  ...
  it('GET /api/sybil/suggested-targets returns bounty targets from METADATA_DIR graph', async () => {
    const res = await getJson('/api/sybil/suggested-targets?limit=6');
    expect(res.status).toBe(200);
  ...
  it('POST /api/prism/spend rejects client-forged earn sources', async () => {
    const token = makeJwt(ADDRESSES.jwt);
    const res = await postJson('/api/prism/spend', {
      address: ADDRESSES.jwt,
      source: 'blackhole_cleanup',
      amount: 1,
      description: 'Black Hole cleanup verified (100 resolved)',
    }, token);
    expect(res.status).toBe(400);
    expect(res.body).toMatchObject({ error: 'Invalid spend source' });
  });

  it('POST /api/prism/spend records a forge purchase notification in inbox', async () => {
    const token = makeJwt(ADDRESSES.jwt);
    const res = await postJson('/api/prism/spend', {
      address: ADDRESSES.jwt,
      source: 'forge_frame',
      amount: 800,
      itemId: 'frame_nebula',
      description: 'Purchased Nebula Frame',
    }, token);
    expect(res.status).toBe(200);
    ...
    const purchase = notifications.find((notification) => notification.message === 'Purchased Nebula Frame (-800 Coins)');
    expect(purchase).toBeTruthy();
    expect(purchase?.type).toBe('system');
    expect(purchase?.meta).toMatchObject({
      source: 'forge_frame',
      itemId: 'frame_nebula',
      spent: 800,
    });
  ...
  it('vault stake and unstake create inbox notifications', async () => {
    const token = makeJwt(ADDRESSES.jwt);

    const stake = await postJson('/api/prism/vault/stake', {
      amount: 10_000,
      tier: 'bronze',
      lockDays: 7,
    }, token);
    ...
    const stakeNotification = notifications.find((notification) => (notification.meta as Record<string, unknown>)?.source === 'vault_stake');
    expect(stakeNotification).toBeTruthy();
    expect(stakeNotification?.message).toContain('Vault stake started');
    expect(stakeNotification?.meta).toMatchObject({
      source: 'vault_stake',
      amount: 10_000,
      tier: 'bronze',
      lockDays: 7,
    ...
    const unstakeNotification = notifications.find((notification) => (notification.meta as Record<string, unknown>)?.source === 'vault_unstake');
    expect(unstakeNotification).toBeTruthy();
    expect(unstakeNotification?.message).toContain('Vault unstaked early');
    expect(unstakeNotification?.meta).toMatchObject({
