...
describe.sequential('route integration tests', () => {
  ...
  it('GET /api/prism/vault/status returns structured response', async () => {
    const res = await getJson(`/api/prism/vault/status?address=${ADDRESSES.known}`, makeJwt(ADDRESSES.known));
  ...
  it('POST /api/quiz/answer with the correct answer awards quiz coins', async () => {
    ...
    const beforeBalance = await getJson(`/api/prism/balance?address=${ADDRESSES.jwt}`, token);
    expect(beforeBalance.status).toBe(200);
    const beforeAmount = (beforeBalance.body as Record<string, number>).balance;

    const res = await postJson('/api/quiz/answer', { id: qBody.id, answer, address: ADDRESSES.jwt }, token);
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ correct: true, earned: 5 });

    const afterBalance = await getJson(`/api/prism/balance?address=${ADDRESSES.jwt}`, token);
    expect(afterBalance.status).toBe(200);
    const afterAmount = (afterBalance.body as Record<string, number>).balance;
    expect(afterAmount).toBe(beforeAmount + 5);
  });
  ...
  it('GET /api/daily-limits or equivalent returns structured response', async () => {
    const res = await getJson(`/api/daily-limits?address=${ADDRESSES.known}`, makeJwt(ADDRESSES.known));
