  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const item of items) {
        ...
        const dasScore = parseInt(attrMap.Score, 10);
        const hasLocalIdentityScore = Number.isFinite(Number(wallet.score)) && Number(wallet.score) > 0;
        if (Number.isFinite(dasScore) && dasScore > 0 && dasScore <= 400 && !hasLocalIdentityScore && !wallet.scoreBreakdown) {
          wallet.score = dasScore;
        }
