...
const readIntervalMs = (envName, fallbackMs) => {
  ...
  return Number.isFinite(override) && override >= 10 ? override : fallbackMs;
};

const AUTH_CHALLENGE_CLEANUP_MS = readIntervalMs('PRISM_SCHEDULER_AUTH_CHALLENGE_CLEANUP_MS', 60_000);
const QUIZ_ANSWER_CLEANUP_MS = readIntervalMs('PRISM_SCHEDULER_QUIZ_CLEANUP_MS', 300_000);
const TOURNAMENT_CHECK_MS = readIntervalMs('PRISM_SCHEDULER_TOURNAMENT_CHECK_MS', 60_000);
const WEEKLY_REWARD_CHECK_MS = readIntervalMs('PRISM_SCHEDULER_WEEKLY_REWARD_MS', 60 * 60 * 1000);
const CHALLENGE_EXPIRY_CHECK_MS = readIntervalMs('PRISM_SCHEDULER_CHALLENGE_EXPIRY_MS', 60_000);
const CHALLENGE_CLEANUP_MS = readIntervalMs('PRISM_SCHEDULER_CHALLENGE_CLEANUP_MS', 30 * 60 * 1000);
const CACHE_CLEANUP_MS = readIntervalMs('PRISM_SCHEDULER_CACHE_CLEANUP_MS', 300_000);
...
function startSchedulers(ctx) {
  ...
  }, QUIZ_ANSWER_CLEANUP_MS));

  handles.push(setInterval(checkTournaments, TOURNAMENT_CHECK_MS));
