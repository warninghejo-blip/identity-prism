...
export const PRISM_BALANCE_UPDATED_EVENT = 'prism-balance-updated';
...
function saveLocalBalance(balance: PrismBalance): void {
  ...
  try {
    sessionStorage.setItem('ip_prism_balance', JSON.stringify(balance));
  } catch {
    /* sessionStorage unavailable */
  }
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent<PrismBalance>(PRISM_BALANCE_UPDATED_EVENT, { detail: balance }));
  }
}
