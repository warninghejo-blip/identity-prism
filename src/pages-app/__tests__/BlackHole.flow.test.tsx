...
// ── Wrapper ───────────────────────────────────────────────────────────────────
const Wrapper: React.FC<{ children: React.ReactNode; initialEntries?: string[] }> = ({
  children,
  initialEntries = ['/'],
}) => (
...
describe('BlackHole — disconnected wallet', () => {
  ...
  it('mounts without crashing when wallet not connected', async () => {
    const { default: BlackHolePage } = await import('@/pages-app/BlackHole');
...
describe('BlackHole — connected wallet', () => {
  ...
  it('prioritizes the connected wallet over address-param scans', async () => {
    const { default: BlackHolePage } = await import('@/pages-app/BlackHole');
    await act(async () => {
      render(
        <Wrapper initialEntries={['/blackhole?address=11111111111111111111111111111111']}>
          <BlackHolePage />
        </Wrapper>,
      );
    });
    ...
    await waitFor(() => {
      ...
      expect(screen.queryByText(/^read-only scan$/i)).not.toBeInTheDocument();
      expect(screen.queryByText(/^wallet ready$/i)).not.toBeInTheDocument();
    });
  });
});
