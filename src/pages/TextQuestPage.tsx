    ...
    <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] backdrop-blur-sm overflow-hidden hover:bg-white/[0.05] transition-all">
      ...
      <div className="p-4 pb-2 flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[8px] font-black px-1.5 py-0.5 rounded uppercase" style={{
              color: diffColor,
      ...
      <div className="px-4 pb-3 pt-1 flex items-center justify-between">
        <div className="flex items-center gap-3 text-white/20 text-[10px]">
          <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {quest.estimatedTime}</span>
          {isCompleted && (
...
export default function TextQuestPage() {
  ...
  const handleStart = useCallback((quest: TextQuest) => {
            ...
            {Object.keys(questState.variables).length > 0 && (
              <div className="flex gap-2 flex-wrap">
                {Object.entries(questState.variables).map(([key, value]) => (
                  <div key={key} className="px-2.5 py-1 rounded-lg bg-white/[0.04] border border-white/[0.06] text-[10px]">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <ChevronRight className={`w-4 h-4 flex-shrink-0 ${locked ? 'text-white/10' : 'text-purple-400/50 group-hover:text-purple-400 group-hover:translate-x-0.5'} transition-all`} />
                        <div className="flex-1">
                          <span className="text-white/80 text-sm">{choice.text}</span>
                          {choice.skillCheck && (
                            <span className={`ml-2 text-[9px] px-1.5 py-0.5 rounded ${locked ? 'text-red-400/60 bg-red-500/10' : 'text-green-400/60 bg-green-500/10'}`}>
                              {locked ? <Lock className="w-2.5 h-2.5 inline mr-0.5" /> : <Zap className="w-2.5 h-2.5 inline mr-0.5" />}
            ...
            {currentNode.isEnding && done && (
              <div className="space-y-4">
                <div className="text-center p-4 rounded-xl bg-gradient-to-b from-purple-500/10 to-transparent border border-purple-500/20">
                  ...
                  <p className="text-white/30 text-xs">Ending: {currentNode.id.replace('ending_', '').replace(/_/g, ' ')}</p>
                  {currentNode.reward && (
