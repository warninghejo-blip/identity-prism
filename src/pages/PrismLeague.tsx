...
const formatPoints = (pts: number) => safeNumber(pts).toLocaleString();
...
function getChallengeJwt(address?: string): string | null {
  if (address) return getCachedJwt(address);
  try {
    const raw = sessionStorage.getItem('ip_auth_jwt');
    if (!raw) return null;
    const parsed = JSON.parse(raw) as { token: string; address: string; expiresAt: number };
    if (parsed.expiresAt > Date.now() + 60_000 && parsed.address) return getCachedJwt(parsed.address);
...
const PrismLeague = () => {
  ...
  useEffect(() => {
    ...
    const currentBest =
      gameMode === 'destroyer'
        ? defenderStats.gamesPlayed > 0 && Number.isFinite(defenderStats.bestScore)
          ? defenderStats.bestScore
          : 0
        : gameMode === 'gravity'
          ? gravityLeaderboard.find((e) => e.address === playerAddr)?.score || 0
          : playerStats.gamesPlayed > 0 && Number.isFinite(playerStats.bestScore)
