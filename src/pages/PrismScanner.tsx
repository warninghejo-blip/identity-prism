...
import {
  ArrowLeft, Search, Loader2, Swords, ChevronRight, ChevronDown,
  Shield, Wallet, Hash, Layers, Image, Calendar, Activity,
  ExternalLink, Clock, User, AlertTriangle, CheckCircle,
  XCircle, BarChart3, GitBranch, Fingerprint, Copy, Check,
  ArrowDownLeft, Zap,
} from 'lucide-react';
...
function getRecentWallets(): string[] {
  try { return JSON.parse(sessionStorage.getItem(RECENT_WALLETS_KEY) || '[]'); } catch { return []; }
}
...
/* ── Types ── */
interface SybilSignal { id: string; name: string; category: string; detected: boolean; weight: number; severity: string; value: string; description: string; }
interface SybilAnalysis { riskScore: number; riskLevel: string; trustScore: number; trustGrade: string; signals: SybilSignal[]; metrics: Record<string, unknown>; behaviorProfile: Record<string, unknown>; }
interface FundingSource { address: string; label: string | null; type: string; totalSolReceived: number; transactionCount: number; percentage: number; }
interface TopProgram { programId: string; name: string; interactions: number; }
...
export default function PrismScanner() {
  ...
  useEffect(() => {
    if (!myAddress) { setCounterparties([]); return; }
  ...
  const fetchSybil = useCallback(async (addr: string) => {
    ...
    if (cached && Date.now() - cached.ts < 1800_000) { setSybilData(cached.data); return; }
    setSybilLoading(true);
    try {
      const r = await fetch(`${BASE()}/api/sybil/analysis?address=${addr}`);
      if (r.status === 429 && cached) { setSybilData(cached.data); return; }
      if (!r.ok) { setSybilData(null); return; }
      const data = await r.json();
      sybilClientCache.set(addr, { data, ts: Date.now() });
      setSybilData(data);
    } catch { setSybilData(null); }
    finally { setSybilLoading(false); }
  }, []);

  const fetchFunding = useCallback(async (addr: string) => {
    try {
      const r = await fetch(`${BASE()}/api/sybil/funding-sources?address=${addr}`);
      if (r.ok) { const d = await r.json(); setFundingSources(d.sources || []); }
    } catch {}
  }, []);

  const handleSearch = useCallback(async (overrideAddr?: string) => {
    const addr = (overrideAddr || query).trim();
    if (!addr || addr.length < 32) { setError('Enter a valid Solana wallet address'); return; }
    setLoading(true); setError(''); setResult(null);
    setSybilData(null); setFundingSources([]);

    const data = await fetchWalletPreview(addr);
    setLoading(false);
    if (!data) { setError('Could not load wallet data'); return; }
    setResult(data);
    ...
    <PageShell>
      <div className="sticky top-0 z-20 backdrop-blur-xl bg-[#050510]/80 border-b border-white/[0.06]">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          <button onClick={() => startFadeTransition(() => goBack(navigate))} className="flex items-center gap-2 text-white/50 hover:text-white transition-colors text-sm min-h-[44px]">
            <ArrowLeft className="w-4 h-4" /> Back
          </button>
          <h1 className="text-sm font-bold bg-gradient-to-r from-cyan-500 to-sky-400 bg-clip-text text-transparent">PRISM SCANNER</h1>
          <div className="w-16" />
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => handleSearch()} disabled={loading} className="bg-cyan-500 hover:bg-cyan-400 text-black font-bold px-6">
        ...
        {!loading && (myAddress || recentWallets.length > 0 || counterparties.length > 0) && (
          <div className="flex flex-wrap gap-2">
            {myAddress && !result && (
              <button onClick={() => { setQuery(myAddress); handleSearch(myAddress); }} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-xs font-bold hover:bg-cyan-500/20 transition-colors">
                <User className="w-3 h-3" /> My Wallet
              </button>
            )}
            {counterparties.slice(0, 5).map(cp => (
              <button key={cp.address} onClick={() => { setQuery(cp.address); handleSearch(cp.address); }} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-purple-500/[0.06] border border-purple-500/[0.12] text-purple-300/60 text-xs font-mono hover:bg-purple-500/[0.12] transition-colors">
                <GitBranch className="w-3 h-3" /> {cp.address.slice(0, 4)}...{cp.address.slice(-4)}
              </button>
            ))}
            {recentWallets.filter(a => a !== myAddress && !counterparties.some(c => c.address === a)).slice(0, 4).map(addr => (
              <button key={addr} onClick={() => { setQuery(addr); handleSearch(addr); }} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white/50 text-xs font-mono hover:bg-white/[0.08] transition-colors">
          ...
          <div className="space-y-3 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="glass-card overflow-hidden" style={{ borderColor: `${tierColor}20` }}>
                ...
                <div className="flex items-center gap-4">
                  <div className="relative flex-shrink-0">
                    <svg width="68" height="68" viewBox="0 0 68 68">
                      ...
                      <circle cx="34" cy="34" r="28" fill="none" stroke={tierColor} strokeWidth="4" strokeLinecap="round"
                        strokeDasharray={`${Math.min(displayScore / maxScore, 1) * 2 * Math.PI * 28} ${2 * Math.PI * 28}`}
                        transform="rotate(-90 34 34)" style={{ filter: `drop-shadow(0 0 6px ${tierColor}80)` }} />
                      <text x="34" y="31" textAnchor="middle" fill={tierColor} fontSize="15" fontWeight="bold" fontFamily="monospace">{displayScore}</text>
                      <text x="34" y="42" textAnchor="middle" fill="rgba(255,255,255,0.2)" fontSize="8">/{maxScore}</text>
                    </svg>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <p className="text-white/40 text-xs font-mono truncate">{result.address.slice(0, 6)}...{result.address.slice(-4)}</p>
                      <CopyButton text={result.address} />
                    </div>
                    <p className="font-bold text-lg" style={{ color: tierColor }}>{TIER_LABELS[displayTier] ?? displayTier.toUpperCase()}</p>
              ...
              <div className="flex border-t border-white/[0.06]">
                <button onClick={() => navigate(`/?address=${result.address}`, { state: { openCard: true } })}
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-bold transition-all hover:bg-white/[0.03]" style={{ color: tierColor }}>
                  <ExternalLink className="w-4 h-4" /> View Full Card
                </button>
                {myAddress && result.address !== myAddress && (
                  <button onClick={() => navigate(`/compare?a=${myAddress}&b=${result.address}`)}
                  ...
                  {(result.topPrograms as TopProgram[]).slice(0, 6).map((p, i) => {
                    ...
                    return (
                      <div key={p.programId || i} className="flex items-center gap-3">
                        <span className="text-xs text-white/60 font-bold w-24 truncate">{p.name || p.programId.slice(0, 8)}</span>
                        <div className="flex-1 h-2 bg-white/[0.04] rounded-full overflow-hidden">
                          <div className="h-full rounded-full bg-gradient-to-r from-cyan-500/60 to-cyan-400/40" style={{ width: `${(p.interactions / maxInt) * 100}%` }} />
                        </div>
...
/* ═══════════════════════════════════════════════
   SYBIL REPORT PANEL
   ═══════════════════════════════════════════════ */
function SybilReportPanel({ data, loading, fundingSources }: { data: SybilAnalysis | null; loading: boolean; fundingSources: FundingSource[] }) {
  ...
  const riskColor = data.riskScore >= 75 ? '#ef4444' : data.riskScore >= 50 ? '#f97316' : data.riskScore >= 30 ? '#eab308' : data.riskScore >= 10 ? '#3b82f6' : '#22c55e';
  const gradeColor = data.trustScore >= 80 ? '#22c55e' : data.trustScore >= 60 ? '#3b82f6' : data.trustScore >= 40 ? '#eab308' : '#ef4444';
  ...
  const categories = [
    { key: 'behavioral', label: 'Behavioral', signals: behavioral, color: '#a78bfa', icon: <BarChart3 className="w-3.5 h-3.5" /> },
    { key: 'financial', label: 'Financial', signals: financial, color: '#fbbf24', icon: <Wallet className="w-3.5 h-3.5" /> },
    { key: 'network', label: 'Network', signals: network, color: '#34d399', icon: <GitBranch className="w-3.5 h-3.5" /> },
  ];
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="glass-card p-4">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-shrink-0">
            <svg width="76" height="76" viewBox="0 0 76 76">
              ...
              <circle cx="38" cy="38" r="30" fill="none" stroke={gradeColor} strokeWidth="5" strokeLinecap="round"
                strokeDasharray={`${(data.trustScore / 100) * 2 * Math.PI * 30} ${2 * Math.PI * 30}`}
                transform="rotate(-90 38 38)" style={{ filter: `drop-shadow(0 0 8px ${gradeColor}60)` }} />
              <text x="38" y="35" textAnchor="middle" fill={gradeColor} fontSize="18" fontWeight="bold" fontFamily="monospace">{data.trustGrade}</text>
              <text x="38" y="48" textAnchor="middle" fill="rgba(255,255,255,0.2)" fontSize="9">{data.trustScore}/100</text>
            </svg>
          </div>
          ...
          <div className="flex-1 flex justify-center">
            <svg width="130" height="130" viewBox="0 0 130 130">
              {/* Grid rings */}
              {[0.33, 0.66, 1].map(r => (
                <polygon key={r} points={radarAxes.map((_, i) => {
              ...
              {radarAxes.map((_, i) => {
                const a = (i / radarAxes.length) * Math.PI * 2 - Math.PI / 2;
                return <line key={i} x1="65" y1="65" x2={65 + Math.cos(a) * 45} y2={65 + Math.sin(a) * 45} stroke="rgba(255,255,255,0.06)" strokeWidth="0.5" />;
              })}
              {/* Data polygon */}
              <polygon points={radarAxes.map((ax, i) => {
                const a = (i / radarAxes.length) * Math.PI * 2 - Math.PI / 2;
                const r = Math.max(0.05, ax.value);
                return `${65 + Math.cos(a) * 45 * r},${65 + Math.sin(a) * 45 * r}`;
              }).join(' ')} fill={`${gradeColor}20`} stroke={gradeColor} strokeWidth="1.5" />
              {/* Data dots + labels */}
              {radarAxes.map((ax, i) => {
                const a = (i / radarAxes.length) * Math.PI * 2 - Math.PI / 2;
                const r = Math.max(0.05, ax.value);
                ...
                return (
                  <g key={ax.label}>
                    ...
                    <text x={lx} y={ly + 3} textAnchor="middle" fill="rgba(255,255,255,0.35)" fontSize="7" fontWeight="bold">{ax.label}</text>
                  </g>
                );
              })}
            </svg>
        ...
        <div className="mt-3 pt-3 border-t border-white/[0.06]">
          <div className="flex items-center justify-between">
            <span className="text-sm font-bold" style={{ color: riskColor }}>Risk: {data.riskScore}/100 — {data.riskLevel.toUpperCase()}</span>
            <span className="text-xs text-white/25">{flagged.length}/{data.signals.length} flagged</span>
          </div>
          <div className="flex flex-wrap gap-1.5 mt-2">
            {metrics.hasSolDomain && <span className="px-2 py-0.5 rounded text-[10px] font-bold text-emerald-400 bg-emerald-400/10 border border-emerald-400/20"><CheckCircle className="w-3 h-3 inline mr-0.5" />.sol</span>}
            {Array.isArray(metrics.defiCategories) && (metrics.defiCategories as string[]).map(c => (
              <span key={c} className="px-2 py-0.5 rounded text-[10px] font-bold text-cyan-400/60 bg-cyan-400/5">{c}</span>
            ))}
            {Number(metrics.trustBonus) > 0 && <span className="px-2 py-0.5 rounded text-[10px] font-bold text-emerald-400 bg-emerald-400/5">bonus -{String(metrics.trustBonus)}</span>}
          </div>
        </div>
      </div>
      ...
      {categories.map(cat => {
        ...
        return (
          <div key={cat.key} className="glass-card overflow-hidden">
            <button onClick={() => setExpandedCategory(isExpanded ? null : cat.key)} className="w-full flex items-center gap-2 px-4 py-3 text-left hover:bg-white/[0.02] transition-colors">
              <span style={{ color: cat.color }}>{cat.icon}</span>
              <span className="text-xs font-bold text-white/60 flex-1">{cat.label}</span>
              {catFlagged > 0
                ? <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-red-500/15 text-red-400">{catFlagged} flagged</span>
                : <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-emerald-500/10 text-emerald-400">clean</span>}
              {isExpanded ? <ChevronDown className="w-3.5 h-3.5 text-white/30" /> : <ChevronRight className="w-3.5 h-3.5 text-white/30" />}
            </button>
            {isExpanded && (
              <div className="px-4 pb-3 space-y-1.5">
                {cat.signals.map(sig => (
                  <div key={sig.id} className={`flex items-start gap-2 p-2 rounded-lg ${sig.detected ? 'bg-red-500/[0.06]' : 'bg-white/[0.02]'}`}>
                    {sig.detected ? (sig.severity === 'danger' ? <XCircle className="w-3.5 h-3.5 text-red-400 mt-0.5 flex-shrink-0" /> : <AlertTriangle className="w-3.5 h-3.5 text-amber-400 mt-0.5 flex-shrink-0" />) : <CheckCircle className="w-3.5 h-3.5 text-emerald-400/50 mt-0.5 flex-shrink-0" />}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className={`text-xs font-bold ${sig.detected ? 'text-white/80' : 'text-white/40'}`}>{sig.name}</span>
                        <span className="text-[10px] text-white/20 font-mono">{sig.value}</span>
                        {sig.detected && <span className="text-[10px] text-red-400/60 font-mono ml-auto">+{sig.weight}</span>}
                      </div>
      ...
      {(metrics.hourBuckets || metrics.hourEntropy != null) && (
        <div className="glass-card p-4">
          <p className="text-xs font-bold text-white/50 mb-3 flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-white/30" /> Activity Heatmap</p>
          <div className="flex gap-px items-end" style={{ height: 40 }}>
            {(() => {
              ...
              return Array.from({ length: 24 }, (_, h) => {
                ...
                return <div key={h} className="flex-1 rounded-t-sm" style={{ height: `${Math.max(8, ratio * 100)}%`, background: `rgba(${isBot ? '239,68,68' : '34,211,238'},${0.15 + ratio * 0.55})` }} title={`${h}:00 — ${count} txs`} />;
              });
            })()}
          </div>
          <div className="flex justify-between mt-1">
            {['0:00', '6:00', '12:00', '18:00', '23:00'].map(l => <span key={l} className="text-[9px] text-white/15">{l}</span>)}
          </div>
                ...
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day, i) => {
                  ...
                  return (
                    <div key={day} className="flex-1 flex flex-col items-center gap-0.5">
                      <div className="w-full rounded-t-sm" style={{ height: `${Math.max(4, ratio * 100)}%`, background: `rgba(${isWe ? '168,85,247' : '34,211,238'},${0.15 + ratio * 0.5})` }} title={`${day}: ${count}`} />
          ...
          <div className="space-y-1.5">
            {fundingSources.slice(0, 6).map(s => (
              <div key={s.address} className="flex items-center gap-2 py-1.5 px-2 rounded-lg hover:bg-white/[0.03]">
                <div className="w-2 h-2 rounded-full" style={{ background: s.percentage > 50 ? '#ef4444' : s.percentage > 20 ? '#f97316' : '#22c55e' }} />
                <span className="text-xs font-mono text-white/40 truncate flex-1">{s.label || `${s.address.slice(0, 4)}...${s.address.slice(-4)}`}</span>
                <span className="text-xs font-bold text-white/50 tabular-nums">{s.totalSolReceived.toFixed(2)} SOL</span>
