...
import { readBlackHolePrefetch, writeBlackHolePrefetch, type BlackHolePrefetchToken } from '@/hooks/useBlackHolePrefetch';
...
const BLACKHOLE_CLASSIFICATION_CACHE_KEY = 'identity-prism:blackhole-classification:v1';
const BLACKHOLE_CLASSIFICATION_CACHE_TTL_MS = 60 * 60 * 1000;
...
const fetchParsedTokenAccountsWithRetry = async (owner: PublicKey, programId: PublicKey) => {
  const retryDelaysMs = [300, 800, 1_600, 2_500];
  let lastError: unknown;
  for (let attempt = 0; attempt <= retryDelaysMs.length; attempt += 1) {
    try {
      return await fetchParsedTokenAccounts(owner, programId);
    } catch (error) {
      lastError = error;
      if (attempt === retryDelaysMs.length) break;
      console.warn('[BlackHole] token account scan retrying', {
  ...
  throw lastError instanceof Error ? lastError : new Error('Token account scan failed');
};

const toBlackHolePrefetchTokens = (tokens: TokenAccount[]): BlackHolePrefetchToken[] =>
  tokens.map((token) => ({
    ...
    data: {
      ...
      info: {
        mint: token.mint,
        state: token.frozen ? 'frozen' : 'initialized',
        tokenAmount: {
...
type CachedBlackHoleClassification = {
  timestamp: number;
  owners: Record<
    string,
    Record<
...
const readBlackHoleClassificationCache = (owner: string) => {
  if (typeof window === 'undefined') return {};
  try {
    const parsed = JSON.parse(window.localStorage.getItem(BLACKHOLE_CLASSIFICATION_CACHE_KEY) || '{}') as CachedBlackHoleClassification;
    if (!parsed?.timestamp || Date.now() - parsed.timestamp > BLACKHOLE_CLASSIFICATION_CACHE_TTL_MS) return {};
    return parsed.owners?.[owner] ?? {};
  } catch {
    return {};
  }
};

const writeBlackHoleClassificationCache = (owner: string, tokens: TokenAccount[]) => {
  if (typeof window === 'undefined') return;
  try {
    const parsed = JSON.parse(window.localStorage.getItem(BLACKHOLE_CLASSIFICATION_CACHE_KEY) || '{}') as CachedBlackHoleClassification;
    const owners = parsed.owners && typeof parsed.owners === 'object' ? parsed.owners : {};
    const current = owners[owner] ?? {};
    tokens.forEach((token) => {
      if (!token.assetStatus || token.assetStatus === 'unknown') return;
      current[token.mint] = {
        status: token.assetStatus,
        reason: token.protectReason,
        isCandidate: token.isCandidate,
      };
    });
    owners[owner] = current;
    window.localStorage.setItem(
      BLACKHOLE_CLASSIFICATION_CACHE_KEY,
      JSON.stringify({ timestamp: Date.now(), owners } satisfies CachedBlackHoleClassification),
    );
  } catch {
    /* cache is opportunistic */
  }
...
const getBlackHoleImageUrl = (image?: string | null) => {
  ...
  if (base && normalized.startsWith(`${base}/api/blackhole/image`)) return normalized;
...
const fetchFallbackPrices = async (mints: string[]): Promise<Map<string, number>> => {
  ...
  // 1) Jupiter proxy — batched through our backend to avoid CORS quirks.
  try {
    const base = getApiBase();
...
const buildSwapTransaction = async (userPublicKey: string, quoteResponse: SwapQuoteResponse) => {
  ...
  // Keep the request alive while the wallet approval flow is open.
...
const VALUE_THRESHOLD_USD_FALLBACK = 0.25;
...
const isResolvableFungibleWithBalance = (token: TokenAccount) =>
  !token.frozen && token.isNft !== true && token.amount > 0n;

const classifyAsset = (token: TokenAccount): { status: AssetStatus; reason?: string } => {
  if (token.closeable === false && !isResolvableFungibleWithBalance(token)) {
...
const hasKnownValue = (token: TokenAccount) =>
  token.valueSol !== null && token.valueSol !== undefined;

const canTreatAsBurnableWithCurrentData = (token: TokenAccount) => {
  if (token.closeable === false) return true;
  if (token.uiAmount === 0) return true;
  if (token.isNft) {
    if (token.collectionId || token.collectionSymbol || token.collectionName) return false;
    return hasKnownValue(token) || token.marketStatus === 'not_listed';
  }
  return hasKnownValue(token);
};

const getUsdValueSignal = (token: TokenAccount) => {
  if (token.valueUsd !== null && token.valueUsd !== undefined) return token.valueUsd;
  if (token.priceUsd === null || token.priceUsd === undefined) return null;
  return token.isNft ? token.priceUsd : token.priceUsd * token.uiAmount;
};

const classifyIncompleteAsset = (token: TokenAccount): { status: AssetStatus; reason?: string } | null => {
  const usdValue = getUsdValueSignal(token);
  if (token.isNft) {
    if (token.collectionId || token.collectionSymbol || token.collectionName || (usdValue !== null && usdValue > 0)) {
      return { status: 'valuable', reason: 'NFT requires price review' };
    }
  }
  if (!token.isNft && usdValue !== null && usdValue > VALUE_THRESHOLD_USD_FALLBACK) {
    return { status: 'valuable', reason: `Worth ~$${usdValue.toFixed(2)}` };
  }
  return null;
};

const shouldRetryIncompleteClassification = (token: TokenAccount) => {
  if (token.closeable === false) return false;
  if (!token.assetStatus || token.assetStatus === 'unknown') return true;
  if (token.isNft && token.marketStatus === 'unknown' && Boolean(token.collectionId || token.collectionName || token.collectionSymbol)) {
    return true;
  }
  if (!token.isNft && token.uiAmount > 0 && token.priceUsd == null && !hasKnownValue(token) && token.uiAmount >= 0.0001) {
    return true;
  }
  return false;
};

const finalizeIncompleteClassification = (token: TokenAccount) => {
  if (!shouldRetryIncompleteClassification(token)) return;
  token.assetStatus = 'valuable';
  token.protectReason = token.protectReason ?? 'Manual review required: metadata or price data incomplete';
...
const BlackHole = () => {
  ...
  }, [connecting, setWalletModalVisible]);
  ...
  const [shieldedBurnConfirm, setShieldedBurnConfirm] = useState<TokenAccount | null>(null);
  const confirmedShieldedBurn = useRef(false);

  const isSelectableToken = useCallback((token: TokenAccount) => {
    if (!canResolveScan) return false;
    if (token.closeable === false && !isResolvableFungibleWithBalance(token)) return false;
    if (!token.assetStatus) return false;
    if (token.assetStatus === 'unknown') return false;
    // Shielded SPL tokens remain blocked — only shielded NFTs are user-burnable (with confirmation)
    if (token.assetStatus === 'protected' && !token.isNft) return false;
    if (token.isNft && token.assetStatus === 'valuable') return false;
    return true;
  }, [canResolveScan]);

  const shieldedNftsInSelection = useMemo(
    () => tokens.filter(
      (t) =>
        selectedTokens.has(t.pubkey.toBase58()) &&
        t.isNft &&
        t.assetStatus === 'protected'
    ),
    [tokens, selectedTokens]
  );
  const hasShieldedNftSelected = shieldedNftsInSelection.length > 0;
  const isShieldedNftBatchViolation = hasShieldedNftSelected && selectedTokens.size > 1;
  ...
  const getVisibleTokens = useCallback(
    (list: TokenAccount[] = tokens, showAll = showAllAssets) => {
      let closeable = showAll ? [...list] : list.filter((token) => token.closeable !== false);
      if (!showAll) {
  ...
  const recoverableTokens = useMemo(
    () => tokens.filter((t) => t.closeable !== false && t.assetStatus === 'burnable'),
    [tokens],
  );

  // Total recoverable SOL from all burnable tokens (shown at top before selection)
  const totalRecoverableSol = useMemo(() => {
    return recoverableTokens.reduce((sum, t) => sum + (t.rentSol || 0), 0);
  }, [recoverableTokens]);

  const totalRecoverablePrism = useMemo(
    () =>
      estimateBlackHoleReward(
        recoverableTokens.filter((token) => !token.isNft).length,
        recoverableTokens.filter((token) => token.isNft).length,
        totalRecoverableSol,
      ),
    [recoverableTokens, totalRecoverableSol],
  );

  const fetchTokens = useCallback(async (owner?: PublicKey | null, attempt = 0) => {
    ...
    try {
      ...
      const [splTokens, token2022Tokens] = await Promise.all([
        fetchParsedTokenAccountsWithRetry(targetOwner, TOKEN_PROGRAM_ID),
        fetchParsedTokenAccountsWithRetry(targetOwner, TOKEN_2022_PROGRAM_ID),
      ]);
      ...
      // Use prefetched SOL price if available and fresh
      const freshPrefetch = readBlackHolePrefetch(targetOwner.toBase58());
      const prefetchFresh = freshPrefetch && (Date.now() - freshPrefetch.timestamp < 5 * 60 * 1000);
      let solUsd: number | null;
      if (prefetchFresh && freshPrefetch.solUsd != null) {
        solUsd = freshPrefetch.solUsd;
      } else {
        solUsd = await fetchSolPriceUsd(proxyBase);
      }
      ...
      // Hoist prefetchedPerks so it's accessible in both if/else branches
      const prefetchedPerks = prefetchFresh ? freshPrefetch.identityPerks : null;
      if (heliusUrl && parsedTokens.length > 0) {
        ...
        try {
          // Use prefetched metadata if available and fresh, skip HTTP fetch
          if (prefetchFresh && freshPrefetch.metadata && Object.keys(freshPrefetch.metadata).length > 0) {
            Object.entries(freshPrefetch.metadata).forEach(([mint, metadata]) => {
              metadataMap.set(mint, metadata);
              if (typeof (metadata as { mint?: string })?.mint === 'string') {
                metadataMap.set((metadata as { mint: string }).mint, metadata);
              }
            });
          } else {
        ...
        if (prefetched?.tokenAccounts?.length && metadataMap.size > 0 && solUsd != null) {
          writeBlackHolePrefetch(targetOwner.toBase58(), {
            tokenAccounts: prefetched.tokenAccounts,
            timestamp: Date.now(),
            complete: true,
            programCounts: prefetched.programCounts,
            metadataStatus: 'ready',
            metadata: Object.fromEntries(metadataMap),
            solUsd,
            identityPerks: prefetched.identityPerks ?? null,
          });
        }
        ...
        try {
          const perks = prefetchedPerks ?? await fetchIdentityPerksWithTimeout(targetOwner.toBase58());
          perksFetched = true;
      ...
      updateScanStep('jupiterPriceFallback', 'Checking dust prices & NFT floors');
      ...
      // Build collection lookups before parallel fetch
      ...
      // Parallel: Jupiter fallback prices + all collection market stats
      const jupiterFetch = noPriceMints.length > 0
        ? fetchFallbackPrices([...new Set(noPriceMints)])
        : Promise.resolve(new Map<string, number>());

      const collectionFetch = collectionLookups.size > 0
        ? Promise.all(
        ...
        : Promise.resolve([] as [string, Awaited<ReturnType<typeof fetchCollectionMarketStats>>][]);

      const [jupPrices, collectionStatuses] = await Promise.all([jupiterFetch, collectionFetch]);
      ...
      updateScanStep('collectionMarketStats', 'Applying NFT floors');

      if (collectionLookups.size > 0) {
        const statusMap = new Map(collectionStatuses);
        parsedTokens.forEach((token) => {
      ...
      const classificationCache = readBlackHoleClassificationCache(targetOwner.toBase58());
      parsedTokens.forEach((token) => {
        ...
        const optimisticClassification = classifyAsset(token);
        if (optimisticClassification.status === 'burnable' && !canTreatAsBurnableWithCurrentData(token)) {
          const incompleteClassification = classifyIncompleteAsset(token);
          if (incompleteClassification) {
            token.assetStatus = incompleteClassification.status;
            token.protectReason = incompleteClassification.reason;
            token.isCandidate = false;
            return;
          }
          const cached = classificationCache[token.mint];
          if (cached && cached.status !== 'unknown') {
            token.assetStatus = cached.status;
            token.protectReason = cached.reason ?? 'Cached classification';
            token.isCandidate = cached.status === 'burnable' ? Boolean(cached.isCandidate) : false;
            return;
          }
          token.assetStatus = 'unknown';
          token.protectReason = 'Awaiting price and market data';
          token.isCandidate = false;
          return;
        }

        const classification = optimisticClassification;
      ...
      parsedTokens.forEach(finalizeIncompleteClassification);
      writeBlackHoleClassificationCache(targetOwner.toBase58(), parsedTokens);
      ...
      writeBlackHolePrefetch(targetOwner.toBase58(), {
        tokenAccounts: toBlackHolePrefetchTokens(parsedTokens),
        timestamp: Date.now(),
        complete: true,
        programCounts: {
          [TOKEN_PROGRAM_ID.toBase58()]: parsedTokens.filter((token) => token.programId.equals(TOKEN_PROGRAM_ID)).length,
          [TOKEN_2022_PROGRAM_ID.toBase58()]: parsedTokens.filter((token) => token.programId.equals(TOKEN_2022_PROGRAM_ID)).length,
        },
        metadataStatus: Object.keys(prefetchMetadata).length > 0 && solUsd != null ? 'ready' : 'failed',
    ...
    (async () => {
      ...
      for (const token of selected) {
        ...
        if (!isSelectableToken(token)) {
          // Shielded NFT — explicit burn intent confirmed via AlertDialog
          if (token.isNft && token.assetStatus === 'protected') {
            nextPlan[key] = {
              token,
              action: 'burn',
              reason: 'Shielded NFT — irreversible burn confirmed by user',
  ...
  const toggleSelection = (pubkey: string) => {
    ...
    const isAdding = !newSelection.has(pubkey);
    if (isAdding) {
      if (token?.isNft && token?.assetStatus === 'protected' && newSelection.size > 0) {
        toast.error('Shielded NFTs can only be burned one at a time', {
          description: 'Deselect everything else first.',
        });
        return;
      }
      if (hasShieldedNftSelected) {
        toast.error('Cannot mix shielded NFT with other assets', {
          description: 'Deselect the shielded NFT first.',
        });
        return;
      }
    }
    if (newSelection.has(pubkey)) newSelection.delete(pubkey);
    else newSelection.add(pubkey);
  ...
  const handleIncinerate = async () => {
    ...
    if (hasShieldedNftSelected && !confirmedShieldedBurn.current) {
      setShieldedBurnConfirm(shieldedNftsInSelection[0]);
      return;
    }
    confirmedShieldedBurn.current = false;
    ...
    try {
      ...
      const unsafeBurn = burnPlans.find(
        (plan) =>
          !(plan.token.isNft && plan.token.assetStatus === 'protected') &&
          (plan.token.assetStatus === 'valuable' || (plan.token.valueSol ?? 0) > VALUE_THRESHOLD_SOL),
      );
      if (unsafeBurn) {
        throw new Error(
  ...
  const sortedTokens = useMemo(() => {
    ...
    return [...visibleTokens].sort((a, b) => {
      ...
      switch (sortField) {
        case 'status': {
          const order: Record<string, number> = { protected: 0, valuable: 1, burnable: 2, unknown: 3 };
          cmp = (order[a.assetStatus ?? 'unknown'] ?? 3) - (order[b.assetStatus ?? 'unknown'] ?? 3);
          break;
        }
        case 'value':
  ...
  const summary = useMemo(() => {
    ...
    const valuableCount = tokens.filter((t) => t.assetStatus === 'valuable' || classifyIncompleteAsset(t)?.status === 'valuable').length;
  ...
  const getTokenDecision = useCallback(
    (token: TokenAccount) => {
      ...
      if (token.assetStatus === 'valuable') {
        return {
          label: 'Caution',
          detail: token.protectReason || 'Valuable asset requires review',
          className: 'bg-amber-950/25 text-amber-300 ring-1 ring-amber-500/15',
          icon: AlertTriangle,
        };
      }
        ...
        {!ownerPublicKey ? (
          ...
          <div className="blackhole-panel space-y-6">
            ...
            {canResolveScan && selectedTokens.size > 0 && (
              <div className="flex flex-col gap-3 rounded-2xl border border-zinc-800/70 bg-zinc-950/65 p-3 sm:flex-row sm:items-center sm:justify-between sm:p-4">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.14em] text-zinc-500">
                    {isPlanning && <Loader2 className="h-3.5 w-3.5 animate-spin text-cyan-300" aria-hidden="true" />}
                    <span>{isPlanning ? 'Analyzing selection' : `${selectedTokens.size} selected`}</span>
                  </div>
                  <div className="mt-1 flex flex-wrap items-baseline gap-x-2 gap-y-1 font-mono text-sm font-semibold text-zinc-100">
                    <span>
                      <span className="mr-1 text-[9px] font-black uppercase tracking-[0.12em] text-cyan-300/70">Return</span>
                      ~{parseFloat(summary.netReturn.toFixed(4))} SOL
                    </span>
                    <span className="text-zinc-600">·</span>
                    <span>
                      <span className="mr-1 text-[9px] font-black uppercase tracking-[0.12em] text-violet-300/70">PRISM</span>
                      +{summary.estimatedReward}
                    </span>
                    {solPriceUsd && (
                      <span className="ml-2 text-xs font-normal text-zinc-500">
                        {formatUsd(summary.netReturn * solPriceUsd)}
                      </span>
                    )}
                  </div>
                  {isShieldedNftBatchViolation && (
                    <div className="mt-1 text-[11px] text-amber-300">
                      Execute one shielded NFT at a time.
                    </div>
                  )}
                </div>
                <Button
                  onClick={handleIncinerate}
                  disabled={isBurning || isPlanning || summary.totalAccounts === 0 || isShieldedNftBatchViolation}
                  className="h-12 w-full rounded-full bg-gradient-to-r from-cyan-400 via-violet-500 to-fuchsia-500 px-6 text-sm font-black text-slate-950 shadow-[0_14px_42px_rgba(167,139,250,0.28)] transition-colors duration-200 hover:from-cyan-300 hover:via-violet-400 hover:to-fuchsia-400 disabled:opacity-50 sm:w-auto"
                >
                  ...
                  EXECUTE {summary.totalAccounts} &rarr; ~{parseFloat(summary.netReturn.toFixed(4))} SOL
                </Button>
              </div>
            )}
            ...
            {tokens.length > 0 && totalRecoverableSol > 0 && selectedTokens.size === 0 && (
              <div className="rounded-2xl border border-zinc-800/70 bg-zinc-950/45 p-3">
                <div className="text-[10px] font-bold uppercase tracking-[0.14em] text-zinc-500">
                  Salvage summary
                </div>
                <div className="mt-2 grid grid-cols-2 gap-2 rounded-xl border border-cyan-500/15 bg-cyan-500/[0.04] px-3 py-2 text-center">
                  <div className="flex flex-col items-center">
                    <div className="text-[9px] font-bold uppercase tracking-[0.14em] text-cyan-300/70">Return</div>
                    <div className="mt-0.5 font-mono text-sm font-semibold text-zinc-100">
                      ~{parseFloat(totalRecoverableSol.toFixed(4))} SOL
                    </div>
                  </div>
                  <div className="flex flex-col items-center">
                    <div className="text-[9px] font-bold uppercase tracking-[0.14em] text-violet-300/70">PRISM</div>
                    <div className="mt-0.5 font-mono text-sm font-semibold text-zinc-100">
                      +{totalRecoverablePrism}
                    </div>
                  </div>
                </div>
                <div className="mt-2 text-[10px] leading-relaxed text-zinc-600">
                  {canResolveScan
                    ? 'Select assets to stage automatic cleanup routing.'
                    : isReadOnlyScan
            ...
            <div className="blackhole-controls">
              <Button
                ...
                variant="outline"
                size="sm"
                className="blackhole-rescan-button border-zinc-800 text-zinc-300 hover:bg-zinc-900 hover:text-zinc-100"
              >
                {isLoading ? (
                              ...
                              decision.label === 'Swap'
                                ...
                                : decision.label === 'Close'
                                  ...
                                  : decision.label === 'Protected'
                                    ? 'text-emerald-400'
                                    : decision.label === 'Quarantine' || decision.label === 'Caution'
                                      ? 'text-amber-400'
                                      : 'text-red-400/70'
                            }`}
                            aria-hidden="true"
