...
const BLACKHOLE_METADATA_CACHE_KEY = 'identity-prism:blackhole-metadata:v4';
...
const fetchCollectionMarketStats = async (
  ...
  try {
    ...
    return { status, floorSol, source, listedCount, tensorUrl: data?.tensorUrl ?? null, meUrl: data?.meUrl ?? null };
  } catch {
    return { status: 'unknown' };
  }
};
    ...
    try {
      ...
      for (let index = 0; index < burnBatches.length; index += 1) {
        ...
        try {
          ...
          // (the reported "cancel → it asks to sign again, and signs anyway" bug).
          ...
          for (let retryIndex = 0; retryIndex < batch.chunk.length; retryIndex += 1) {
            ...
            try {
              ...
              // Reject during the per-asset retry must also abort, not roll on to the next asset.
      ...
      // PRISM reward claim is a Seeker-only privilege; web flow skips it entirely.
      // The burn / close itself already returned the SOL rent to the wallet, which is the visible result.
      const claimedNetResolvedSol = 0;
      ...
      toast.success('Resolution complete!', {
        description: `Resolved ${resolvedCount} asset${resolvedCount === 1 ? '' : 's'}${skippedSuffix} · ~${(claimedNetResolvedSol || estimatedNetSol).toFixed(4)} SOL recovered`,
      });
        ...
        {!ownerPublicKey ? (
          <div className="blackhole-panel flex flex-col items-center gap-5 py-8 text-center sm:gap-6 sm:py-12">
            ...
            <p className="text-sm font-medium text-cyan-200/80">
              Connect your wallet first — use the <span className="font-semibold text-cyan-300">Connect Wallet</span> button in the top-right menu.
            </p>
          ...
          <div className="blackhole-panel space-y-6">
              ...
              <div className="flex shrink-0 flex-wrap items-center gap-2">
                {!connectedMatchesScan && (
                  <span className="text-xs text-zinc-400">
                    {isReadOnlyScan
                      ? 'Connect the matching wallet via the top-right menu'
                      : 'Connect your wallet via the top-right menu'}
                  </span>
                )}
              </div>
            </div>
            ...
            {canResolveScan && (
              <div className="manifest-card blackhole-resolution-manifest rounded-2xl border border-zinc-800/70 bg-zinc-950/45 p-3 shadow-[0_14px_36px_rgba(0,0,0,0.22)] backdrop-blur-xl sm:p-4">
                ...
                <div
                  ...
                  title="Estimated Prism Coins from verified cleanup. Web cleanup currently recovers SOL; Seeker reward claim can be capped by daily limits."
                >
                ...
                {isShieldedNftBatchViolation && (
                  <div className="mt-3 rounded-xl border border-amber-400/20 bg-amber-950/24 px-3 py-2 text-[11px] text-amber-300">
            ...
            {tokens.length > 0 && totalRecoverableSol > 0 && selectedTokens.size === 0 && (
              <div className="rounded-2xl border border-zinc-800/70 bg-zinc-950/45 p-3">
                ...
                <div className="mt-2 rounded-xl border border-cyan-500/15 bg-cyan-500/[0.04] px-3 py-2 text-center">
