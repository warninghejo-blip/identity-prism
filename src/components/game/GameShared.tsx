...
export const AST_SHARED_MATS = AST_TEX_PAIRS.map((pair, i) => {
  return new THREE.MeshStandardMaterial({
    ...
    roughness: Math.min(AST_M[i].rou + 0.05, 1),
    metalness: Math.max(AST_M[i].met - 0.02, 0),
...
export function spawnAst(el: number, sx: number, sy: number): AsteroidData {
  ...
  const ca = a + Math.PI + rnd(-0.55, 0.55);
  ...
  return {
    ...
    sy: rnd(0.88, 1.15),
    sz: rnd(0.88, 1.15),
    gi: Math.floor(rnd(0, 6)),
    mi: Math.floor(rnd(0, 4)),
    small,
    active: true,
    wobbleAmp: rnd(0.3, 1.8),
    wobbleFreq: rnd(1.5, 4),
    wobblePhase: rnd(0, 6.28),
    spawnTime: el,
    hp: small ? 1 : 2,
  };
}
...
export function Cam({
  ...
  useFrame(({ camera }, delta) => {
    ...
    if (shake.current > 0.01) {
      sx = (Math.random() - 0.5) * shake.current * 0.6;
      sy = (Math.random() - 0.5) * shake.current * 0.6;
...
const _bhRingGeo = new THREE.RingGeometry(0.92, 1.05, IS_MOBILE ? 24 : 64);
...
export function BHVisuals({ bhRef }: { bhRef: React.MutableRefObject<BHole[]> }) {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < BH_N; i++) {
      ...
      const sc = 1.2 + (w.str / BH_STR) * 0.4;
      g.scale.setScalar(sc);
      if ((g.children[0] as THREE.Mesh).material) {
        const _m = (g.children[0] as THREE.Mesh).material as THREE.MeshBasicMaterial;
        if (_m.opacity !== undefined && _m.opacity !== fade) _m.opacity = fade;
      }
      ((g.children[1] as THREE.Mesh).material as THREE.MeshBasicMaterial).opacity = fade * 0.5;
      ...
      if (spiralMesh) spiralMesh.rotation.z = t * (0.6 + i * 0.15);
      ((g.children[3] as THREE.Mesh).material as THREE.MeshBasicMaterial).opacity =
        fade * 0.1 * (0.8 + Math.sin(t * 1.5 + i) * 0.2);
    }
  });
  return (
    <>
      {Array.from({ length: BH_N }).map((_, i) => (
        ...
        >
          ...
          <mesh geometry={_bhCircleGeo} position={[0, 0, 0.01]}>
            <shaderMaterial
              ref={(el) => {
...
export function Ship({
  ...
  const exSt = useMemo(() => {
    const p = new Float32Array(N * 3);
    const nozzles = profile.exhausts;
    const d: { l: number; ml: number; sp: number; ox: number; oy: number }[] = [];
    for (let i = 0; i < N; i++) {
      const nz = nozzles[i % nozzles.length];
      d.push({
        l: Math.random() * 0.5,
        ml: 0.2 + Math.random() * 0.35,
        sp: 1.2 + Math.random() * 2.5,
        ox: nz.x + (Math.random() - 0.5) * 0.06,
        oy: nz.y,
      });
      p[i * 3] = d[i].ox;
      p[i * 3 + 1] = d[i].oy;
  ...
  useFrame((s, delta) => {
    ...
    if (shieldBub.current) {
      ...
      if (on) {
        (shieldBub.current.material as THREE.MeshBasicMaterial).opacity = 0.12 + Math.sin(t * 4) * 0.05;
        shieldBub.current.scale.setScalar(1 + Math.sin(t * 3) * 0.05);
      }
    }
    if (phaseGlow.current) {
      ...
      if (on) {
        (phaseGlow.current.material as THREE.MeshBasicMaterial).opacity = 0.15 + Math.sin(t * 6) * 0.08;
      }
    }
    ...
    if (exRef.current) {
      ...
      for (let i = 0; i < N; i++) {
        ...
        if (d.l <= 0) {
          ...
          a[i * 3 + 1] -= d.sp * dt;
          a[i * 3] += (Math.random() - 0.5) * dt * 0.6;
          a[i * 3 + 2] += (Math.random() - 0.5) * dt * 0.3;
        }
      }
  ...
  return (
    <group ref={gRef} scale={[scale, scale, scale]}>
      ...
      <mesh ref={shieldBub} visible={false}>
      ...
      <mesh ref={phaseGlow} visible={false}>
      ...
      <points ref={exRef}>
        <bufferGeometry>
          <bufferAttribute attach="attributes-position" args={[exSt.p, 3]} />
        </bufferGeometry>
        <pointsMaterial
          size={0.1}
          color={ac.light}
...
export function Explosion({
  ...
  useFrame((_, delta) => {
    ...
    if (ptRef.current && ptRef.current.geometry?.attributes?.position) {
      ...
      for (let i = 0; i < EXPLODE_N; i++) {
        if (t < 0.02) {
          a[i * 3] = pRef.current.x;
          a[i * 3 + 1] = pRef.current.y;
          a[i * 3 + 2] = 0;
        } else {
          a[i * 3] += st.v[i * 3] * dt;
...
export const PWR_COL: Record<PwrType, string> = {
  shield: '#22d3ee',
  slowmo: '#facc15',
  phase: '#a855f7',
  coin: '#fbbf24',
...
export function PowerUpVisuals({ pwRef }: { pwRef: React.MutableRefObject<PowerUp[]> }) {
  ...
  useFrame((s) => {
    ...
    for (let i = 0; i < PWR_MAX; i++) {
      ...
      const fade = Math.min(fadeIn, fadeOut);
      g.visible = fade > 0.02;
      g.position.set(pw.x, pw.y, Math.sin(t * 2.5 + i * 3) * 0.2);
      g.rotation.y = t * 1.8;
      g.scale.setScalar(1 + Math.sin(t * 3.5 + i * 2) * 0.08);
      ...
      if (halo) {
        ...
        m.color.set(col);
        m.opacity = fade * 0.2 * (0.6 + Math.sin(t * 4) * 0.4);
      }
      ...
      {Array.from({ length: PWR_MAX }).map((_, i) => (
        ...
        >
          ...
          >
            ...
            <mesh>
              <torusGeometry args={[0.32, 0.14, 16, 24]} />
              <meshBasicMaterial toneMapped={false} transparent opacity={0} />
            </mesh>
            <mesh>
...
const _astCounts = new Array(AST_TEX_PAIRS.length).fill(0);
export function AsteroidInstances({
  pool,
  geos,
}: {
...
export function splitAsteroid(pool: AsteroidData[], parent: AsteroidData, elapsedTime: number): void {
  ...
  for (let i = 0; i < pool.length && spawned < count; i++) {
    ...
    frag.rx = Math.random() * 6.28;
    frag.ry = Math.random() * 6.28;
    frag.rz = Math.random() * 6.28;
    frag.rsx = (Math.random() - 0.5) * 5;
    frag.rsy = (Math.random() - 0.5) * 5;
    frag.rsz = (Math.random() - 0.5) * 5;
    frag.sx = 0.85 + Math.random() * 0.3;
    frag.sy = 0.85 + Math.random() * 0.3;
    frag.sz = 0.85 + Math.random() * 0.3;
    frag.gi = Math.floor(Math.random() * 6);
    frag.mi = Math.floor(Math.random() * 4);
    frag.small = true;
    frag.active = true;
...
export interface SmallExplosion {
  x: number;
  y: number;
  t: number;
  active: boolean;
  ...
  useFrame((_, delta) => {
    ...
    for (let p = 0; p < Math.min(poolRef.current.length, MAX_POOL); p++) {
      ...
      if (!pts || !e || !e.active) {
        ...
        initFlags.current[p] = false;
        continue;
      }
      e.t += dt;
      if (e.t > (IS_MOBILE ? 0.5 : 1.0)) {
        e.active = false;
        pts.visible = false;
        initFlags.current[p] = false;
        continue;
      ...
      m.opacity = Math.max(0, 1 - e.t / 0.8);
      m.size = 0.18 + e.t * 0.06;
      m.color.set(e.color);
    }
  });
...
export function createGeos(): THREE.SphereGeometry[] {
  ...
  return [
    makeRockGeo(Math.round(22 * s), 41),
    makeRockGeo(Math.round(20 * s), 67),
    makeRockGeo(Math.round(24 * s), 89),
    makeRockGeo(Math.round(22 * s), 97),
    makeRockGeo(Math.round(20 * s), 131),
    makeRockGeo(Math.round(24 * s), 157),
  ];
}
